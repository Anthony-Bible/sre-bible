...
### Interview Mode
An opt-in Session mode in which the Resume Agent sets aside its normal résumé Q&A and runs a fixed sequence of three SRE incident scenarios, grading each answer via the judge Tool (`evaluate_interview_answer`) and reporting that answer's score, demonstrated concepts, and short feedback. It is a live demonstration of the agent stack rather than a qualification gate: there is no aggregate score, no pass/fail outcome, and no scheduling or contact push. The scenarios are scripted into the agent's instructions, not drawn from a Source, so no Retrieval happens while the mode is active. The frontend exposes it via the `/interview` command and tracks progress in a small demo-mode HUD. The whole feature is gated by the `INTERVIEW_MODE_ENABLED` env var (default off): when disabled the backend refuses to activate the mode regardless of request headers and the frontend hides the command and HUD.

### Scenario
One of the three fixed SRE incident prompts presented during Interview Mode. Two states distinguish the UI counter: a scenario is *in play* once the agent presents it, and *graded* once the judge Tool returns a score for the Viewer's answer. The backend's `interview_progress` SSE event carries the *graded* count; the HUD derives the scenario *in play* as `min(graded + 1, total)` — so it reads "Scenario 1 of 3" before any answer is graded and "✓ Complete · 3 of 3" once all three are graded.

### Restart vs Exit
The two ways to leave a scenario run. *Restart* (`/interview restart`) reseeds a fresh three-scenario run but **stays** in Interview Mode — the frontend sends a one-shot `X-Interview-Reset: true`. *Exit* (`/exit`) clears interview state and **leaves** the mode for standard résumé Q&A — the frontend sends `X-Interview-Mode: false`, drops the HUD immediately, and the backend clears its state lazily on the next turn.
