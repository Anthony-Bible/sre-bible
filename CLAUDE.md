...
| `SESSION_CACHE_ENABLED` | server | Optional kill-switch for the galaxycache session-state cache tier (ADR 0013). `strconv.ParseBool` syntax. **Default `false` (disabled)** — when off, the caching decorator, the `:9091` peer listener, and the peer-refresh goroutine are never constructed and behaviour is bit-for-bit unchanged. Phase 1 caches only the monotonic `turnstile_verified` flag (read-through, fail-open) to remove the DB read on the `/suggestions` verified-gate. Invalid value → default + warn. |
| `MY_POD_IP` | server | Downward-API-injected pod IP (`fieldRef: status.podIP`). Forms the galaxycache self ID (`http://$MY_POD_IP:<port>`), which must match the self entry produced by peer refresh. **Fatal at startup iff `SESSION_CACHE_ENABLED=true` and unset**; ignored when the cache is disabled. |
| `SESSION_CACHE_LISTEN_ADDR` | server | galaxycache peer HTTP listener address (cluster-internal only; never exposed via ingress). Default `:9091` (distinct from `:8080`/`:9090`). Only read when the cache is enabled. |
| `SESSION_CACHE_MAX_BYTES` | server | Galaxy main-cache byte budget per replica. Default `10000000` (10 MB). Invalid/non-positive → default + warn. Only read when the cache is enabled. |
| `SESSION_CACHE_TTL_SECONDS` | server | `WithGetTTL` max time-to-live — **eviction hygiene only, not a correctness mechanism** (an evicted `true` is simply re-fetched; the monotonic flag makes a stale `true` still correct, and `false` is never cached). ~10% jitter is applied to avoid synchronised expiry. Default `3600`. Invalid/non-positive → default + warn. Only read when the cache is enabled. |
| `SESSION_CACHE_PEER_REFRESH_SECONDS` | server | Interval between headless-DNS re-resolves / `universe.Set` peer updates. Default `30` (fits inside `terminationGracePeriodSeconds: 40`). Invalid/non-positive → default + warn. Only read when the cache is enabled. |
| `SESSION_CACHE_HEADLESS_DNS` | server | Headless Service DNS name resolved to discover peer pod IPs. Default `sre-bible-headless.sre-bible.svc.cluster.local`. Only read when the cache is enabled. |
